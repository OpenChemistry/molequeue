                                      ...
                                      IdType moleQueueId)
{
  ...
  if (job.isValid())
    ...
    sendFailedLookupJobResponse(connection, replyTo, packetId);
}
...
{
  PacketType packet = m_jsonrpc->generateLookupJobResponse(req, packetId);
  Message msg(replyTo, packet);
  connection->send(msg);
}
...
void Server::sendFailedLookupJobResponse(Connection *connection,
                                         EndpointId replyTo, IdType packetId)
{
  PacketType packet = m_jsonrpc->generateLookupJobResponse(Job(), packetId);
  Message msg(replyTo, packet);
  connection->send(msg);
}
