 ...
 *
 * JobReferenceBase holds a QPointer to the JobData object it represents. The
 * validity of the JobData pointer can be checked with isValid(), which will
 * return false if the JobData has been destroyed. Subclasses of
 * JobReferenceBase, Job on the Server and JobRequest on the Client, will
 * forward requests to the JobData. Certain methods may cause signals to be
 * emitted from JobManager; these cases will be noted in the method
 * documentation.
 */
...
public:
  ...
  JobReferenceBase(const JobReferenceBase &other)
    : m_jobData(other.m_jobData), m_moleQueueId(other.m_moleQueueId) {}
  ...
  bool isValid() const { return !m_jobData.isNull(); }
...
protected:
  ...
  QPointer<JobData> m_jobData;

private:
  /// Used for debugging invalid references. May change in future.
