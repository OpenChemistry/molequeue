...
LogEntry::LogEntry(QSettings &settings)
  : m_message(settings.value("message").toString()),
    ...
    m_timeStamp(new QDateTime(QDateTime::fromString(
                                settings.value("timeStamp").toString())))
{
}
...
void LogEntry::writeSettings(QSettings &settings) const
{
  ...
  settings.setValue("moleQueueId",
                    static_cast<unsigned long long>(m_moleQueueId));
